<template>
  <b-list-group flush class="config-list" v-if="list">
    <div>
      ...
      <b-list-group-item :ref="`i-${index}`" v-for="(item, index) in list.true" :key="'t-' + index" :class="{'px-3 py-2 hover': true, 'selected': checkIfSelected(item)}" @click="selectConfiguration(item)">
        <p class="mb-0">
          {{item.id}}<br>
          <small>
        ...
        <b-form-checkbox :checked="checkIfSelected(item)" switch size="sm"></b-form-checkbox>   
      </b-list-group-item>
    </div>
    <div class="border-bottom">
      ...
      <b-list-group-item :ref="`i-${index}`" v-for="(item, index) in list.false" :key="'f-' + index" :class="{'px-3 py-2 hover': true, 'selected': checkIfSelected(item)}" @click="selectConfiguration(item)">
        <p class="mb-0">
          {{item.id}}<br>
          <small>
            <span v-for="a in filters.filter((a) => a.isFiltered)" :key="a.key"><b>{{a.friendlyName}}</b>: {{item.attributes[a.key]}}<br></span>
